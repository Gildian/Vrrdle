...
import { getRandomCar, fetchLeaderboard } from '../api/ApiService';
import './Game.css';
...
function Game() {
  ...
  function fetchRandomCar() {
    getRandomCar()
      ...
      .catch(error => {
        ...
        setMessage('Error loading car. Please try again.');
      });
  }
  ...
  const handleGuess = () => {
    ...
    if (guess.trim().toLowerCase() === correctAnswer.toLowerCase()) {
      ...
      }, 2000); // Longer delay to show the correct answer
  ...
  const handleKeyPress = (e) => {
    if (e.key === 'Enter' && !isGameOver && guess.trim()) {
      handleGuess();
    }
  };
